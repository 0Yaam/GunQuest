...
public class PlayerMotor : MonoBehaviour
{
    ...
    private void UpdateMovementAnimator(Vector2 input, float maxSpeed)
    {
        ...
        SetAnimatorFloat(MoveXHash, input.x);
        SetAnimatorFloat(MoveYHash, input.y);
        SetAnimatorFloat(SpeedHash, speed01);
        SetAnimatorBool(IsMovingHash, input.sqrMagnitude > 0.01f);
    ...
    private void UpdateDirectAnimatorState(float speed01)
    {
        ...
        PlayAnimatorState(speed01 > 0.05f ? runStateName : idleStateName);
    }
